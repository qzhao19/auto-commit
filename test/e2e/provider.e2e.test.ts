...
(OPENAI_API_KEY ? describe : describe.skip)("E2E: OpenAI provider", () => {
  test("createProvider returns OpenAIProvider", async () => {
  ...
  test("invoke returns non-empty string from real API", async () => {
    const provider = await createProvider({
      argv: [],
      env: makeEnv({
        AUTOCOMMIT_PROVIDER: "openai",
...
(DEEPSEEK_API_KEY ? describe : describe.skip)("E2E: DeepSeek provider", () => {
  test("createProvider returns OpenAIProvider", async () => {
    const provider = await createProvider({
      argv: [],
      env: makeEnv({
  ...
  test("uses correct DeepSeek baseUrl (api.deepseek.com)", async () => {
    const provider = await createProvider({
      argv: [],
      env: makeEnv({
        AUTOCOMMIT_PROVIDER: "deepseek",
...
const OLLAMA_BASE_URL =
  process.env.E2E_OLLAMA_BASE_URL ?? "http://localhost:11434";
// const OLLAMA_MODEL    = process.env.E2E_OLLAMA_MODEL ?? "gemma:2b";

describe("E2E: Ollama provider", () => {
  test("createProvider returns OpenAIProvider and invoke returns non-empty string", async () => {
  ...
  test("ollama with explicit apiKey override still works", async () => {
    const reachable = await isReachable(`${OLLAMA_BASE_URL}/api/tags`);
    if (!reachable) {
      console.log(`[e2e] skipped: Ollama not reachable at ${OLLAMA_BASE_URL}`);
      return;
